...
st.caption("Python/RK4 dashboard reconstructed from the STELLA midterm model. It is designed for interactive demonstration, not as an official STELLA publisher replacement.")


def line_plot(df: pd.DataFrame, y_cols, title: str, color=None, facet=False):
    if color:
...
def overlay_plot(df: pd.DataFrame, y_col: str, title: str):
    fig = px.line(df, x="time", y=y_col, color="run", title=title, hover_data=["parameter_value"] if "parameter_value" in df.columns else None)
...
dt = st.sidebar.selectbox("DT", [1/64, 1/128, 1/32], index=0, format_func=lambda x: f"{x:.6f}")
...
if task == "Task 1: Baseline":
    ...
    st.info("Take-home message: under the high-transmission and high-mortality baseline, the system tends toward host depletion rather than endemic equilibrium.")
    ...
    st.info("Take-home message: vector-side interventions, especially reducing biting or increasing mosquito mortality, suppress transmission more directly than increasing recovery alone.")
    ...
    col1, col2 = st.columns(2)
    col1.plotly_chart(px.line(comp, x="time", y="S", color="scenario", title="Sick villagers S"), use_container_width=True)
    col2.plotly_chart(px.line(comp, x="time", y="I", color="scenario", title="Infected mosquitoes I"), use_container_width=True)
    col3, col4 = st.columns(2)
    col3.plotly_chart(px.line(comp, x="time", y="M", color="scenario", title="Healthy mosquitoes M"), use_container_width=True)
    col4.plotly_chart(px.line(pest_df, x="time", y=["Pesticide_Effect", "g_new"], title="Pesticide effect and dynamic g"), use_container_width=True)
    ...
    st.info("Take-home message: pesticide strongly suppresses mosquitoes early, but under high transmission it does not substantially reduce the first human infection peak unless paired with contact reduction.")
    ...
    if mode == "Single scenario":
        ...
        metric_cards(summarize(df))
        col1, col2 = st.columns(2)
        col1.plotly_chart(line_plot(df, ["H", "S"], f"Human populations, T={22+w:.1f}°C"), use_container_width=True)
        col2.plotly_chart(line_plot(df, ["M", "I"], f"Mosquito populations, T={22+w:.1f}°C"), use_container_width=True)
        ...
        st.plotly_chart(px.line(thermal, x="Temperature", y=["c(T)", "z(T)"], title="Thermal response curves"), use_container_width=True)
    else:
        ...
        col2.plotly_chart(overlay_plot(overlay_df, "I", "Infected mosquitoes I"), use_container_width=True)
        st.dataframe(summary_df.round(4), use_container_width=True)
    st.info("Take-home message: transmission is highest near the thermal optimum around 25–26°C; extreme warming can reduce transmission efficiency.")
    ...
    t_end = st.sidebar.slider("Time horizon", 5, 100, 20 if option == "z" else 100, 5)
    values = np.linspace(0.0, 1.0, 10)
    overlay_df, summary_df = make_overlay(option, values, t_end=float(t_end), dt=dt)
    ...
    col2.plotly_chart(overlay_plot(overlay_df, "I", "Infected mosquitoes I"), use_container_width=True)
    col3, col4 = st.columns(2)
    col3.plotly_chart(px.line(summary_df, x="parameter_value", y="S_peak", markers=True, title="S peak vs parameter"), use_container_width=True)
    col4.plotly_chart(px.line(summary_df, x="parameter_value", y="S_time_to_peak", markers=True, title="S time to peak vs parameter"), use_container_width=True)
    st.dataframe(summary_df.round(4), use_container_width=True)
    st.info("Take-home message: z controls early human outbreak size and timing, while e mainly affects long-term mosquito growth in this vector-abundant setting.")
